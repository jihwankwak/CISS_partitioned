...
class BaseTrainer:
    ...
    def __init__(self, config, logger, gpu):
        ...
        self.method = config['name']
        self.seed = config['seed']
        self.setting = config['data_loader']['args']['task']['setting']
        ...
        # self.plabel_validation = cfg_trainer['pseudo_val_period']
    ...
    def test(self, epoch=None):
        result = self._test(epoch)
        
        if self.rank == 0:
            log = {}
            log.update(result)
            self._save_checkpoint(self.save_period)
    ...
    def _save_checkpoint(self, epoch):
        ...
        filename = str(self.checkpoint_dir / 'checkpoint-{}-{}-epoch{}-seed{}.pth'.format(self.setting, self.method, epoch, self.seed))
