...
class Resize(object):
    ...
    def __call__(self, img, lbl=None):
        ...
        """
        if lbl is not None:
            return F.resize(img, (self.size, self.size), self.interpolation), F.resize(lbl, (self.size, self.size), InterpolationMode.NEAREST)
        else:
            return F.resize(img, (self.size, self.size), self.interpolation)

    def __repr__(self):
        interpolate_str = _interpolation_to_str[self.interpolation]
...
class Pad(object):
    ...
    def __init__(self, padding, img_fill=255, lb_fill=255, padding_mode='constant'):
        assert isinstance(padding, (numbers.Number, tuple))
        assert isinstance(img_fill, (numbers.Number, str))
        assert isinstance(lb_fill, (numbers.Number, str))
    ...
    def __call__(self, img, lbl=None):
        ...
        """
        if lbl is not None:
            return F.pad(img, self.padding, self.img_fill, self.padding_mode), F.pad(lbl, self.padding, self.lb_fill, self.padding_mode)
        else:
            return F.pad(img, self.padding, self.img_fill, self.padding_mode)
...
class Crop(object):
    """Crop the given PIL Image with a given location
    ...
    """
    def __init__(self, t, l, h, w, padding=0, pad_if_needed=False):
        self.padding = padding
    ...
    def __call__(self, img, lbl):
        ...
        Args:
            ...
            t (int) : Vertical component of the top left corner of the crop box.
            l (int): Horizontal component of the top left corner of the crop box.
            h (int): Height of the crop box.
            w (int): Width of the crop box.
        Returns:
            PIL Image: Cropped image.
        ...
        if lbl is None:
            ...
            if self.pad_if_needed and img.size[0] < size[1]:
                img = F.pad(img, padding=int((1 + size[1] - size[0]) / 2))
            # pad the height if needed
            if self.pad_if_needed and img.size[1] < size[0]:
                img = F.pad(img, padding=int((1 + size[0] - img.size[1]) / 2))
            
            return F.crop(img, t, l, h, w)
        else:
            ...
            if self.pad_if_needed and img.size[0] < size[1]:
                print('a')
                img = F.pad(img, padding=int((1 + size[1] - img.size[0]) / 2))
                lbl = F.pad(lbl, padding=int((1 + size[1] - lbl.size[0]) / 2))

            # pad the height if needed
            if self.pad_if_needed and img.size[1] < size[0]:
                print('b')
                img = F.pad(img, padding=int((1 + size[0] - img.size[1]) / 2))
                lbl = F.pad(lbl, padding=int((1 + size[0] - lbl.size[1]) / 2))

            return F.crop(img, t, l, h, w), F.crop(lbl, t, l, h, w)
        
    def __repr__(self):
        return self.__class__.__name__ + '(padding={1})'.format(self.padding)
