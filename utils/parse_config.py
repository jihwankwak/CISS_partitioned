...
class ConfigParser:
    def __init__(self, config, resume=None, modification=None, run_id=None):
        ...
        # here, self.config['test'] becomes true (since 'test' exists in modificaiton/customrargs)
        self._config = _update_config(config, modification)
        self.resume = resume
        self.run_id = run_id

        # set save_dir where trained model and log will be saved.
        if self.config['linear_probe'] is True:
            idx = str(self.resume).split('/').index('models')
            save_dir = Path('/'.join(str(self.resume).split('/')[:idx]))
        else:
        ...
        try:
            if self.config['test'] is True or self.config['linear_probe'] is True:
            ...
            elif self.config['linear_probe'] is True:
                self._save_dir = save_dir / 'models' / exper_name / f'linearprobe_{run_id}'
                pass
            else:
                raise FileExistsError
            self.save_dir.mkdir(parents=True, exist_ok=exist_ok)
        try:
            ...
            if self.config['test'] is True:
                ...
                self._log_dir = save_dir / 'log' / exper_name / f'linearprobe_{run_id}'
                pass
            else:
                raise FileExistsError
            self.log_dir.mkdir(parents=True, exist_ok=exist_ok)
