...
class Evaluator:
    def __init__(self, num_class, old_classes_idx=None, new_classes_idx=None):
        ...
        self.total_classes_idx = list(set(self.old_classes_idx + self.new_classes_idx))
    ...
    def Pixel_Precision_Class(self):
        Precision_by_class = np.diag(self.confusion_matrix) / self.confusion_matrix.sum(axis=0) * 100
        Precision = np.nanmean(np.nan_to_num(Precision_by_class)[self.total_classes_idx])
        if self.old_classes_idx and self.new_classes_idx:
            old_classes_idx = self.old_classes_idx.copy()
            old_classes_idx.remove(0)
            Precision_bg = np.nanmean(np.nan_to_num(Precision_by_class[0]))
            Precision_old = np.nanmean(np.nan_to_num(Precision_by_class[old_classes_idx]))
            Precision_new = np.nanmean(np.nan_to_num(Precision_by_class[self.new_classes_idx]))
            return {'harmonic': 2 * Precision_old * Precision_new / (Precision_old + Precision_new),
                    'by_class': Precision_by_class, 'old': Precision_old, 'new': Precision_new, 'overall': Precision}
        else:
            return {'overall': Precision, 'by_class': Precision_by_class}
    ...
    def Pixel_Accuracy_Class(self):
        ...
        if self.old_classes_idx and self.new_classes_idx:
            old_classes_idx = self.old_classes_idx.copy()
            old_classes_idx.remove(0)
            Acc_bg = np.nanmean(np.nan_to_num(Acc_by_class[0]))
            Acc_old = np.nanmean(np.nan_to_num(Acc_by_class[old_classes_idx]))
            Acc_new = np.nanmean(np.nan_to_num(Acc_by_class[self.new_classes_idx]))
            return {'harmonic': 2 * Acc_old * Acc_new / (Acc_old + Acc_new),
                    'by_class': Acc_by_class, 'bg':Acc_bg, 'old': Acc_old, 'new': Acc_new, 'overall': Acc}
        else:
    ...
    def Mean_Intersection_over_Union(self):
        ...
        if self.old_classes_idx and self.new_classes_idx:
            old_classes_idx = self.old_classes_idx.copy()
            old_classes_idx.remove(0)
            MIoU_bg = np.nanmean(np.nan_to_num(MIoU_by_class[0]))
            MIoU_old = np.nanmean(np.nan_to_num(MIoU_by_class[old_classes_idx]))
            old_classes_idx.append(0)
            MIoU_bgold = np.nanmean(np.nan_to_num(MIoU_by_class[old_classes_idx]))
            MIoU_new = np.nanmean(np.nan_to_num(MIoU_by_class[self.new_classes_idx]))
            return {'harmonic': 2 * MIoU_old * MIoU_new / (MIoU_old + MIoU_new), 'by_class': MIoU_by_class, 'bg': MIoU_bg, 'old': MIoU_old, 'bgold': MIoU_bgold, 'new': MIoU_new, 'overall': MIoU}
        else:
    ...
    def Confusion_matrix(self):
        return {'confusion': self.confusion_matrix}
