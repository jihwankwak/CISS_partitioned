...
from models.loss import WBCELoss, KDLoss, ACLoss, BCELoss_DKD
from data_loader import CLS_INFO
...
class Trainer_base(BaseTrainer):
    ...
    def __init__(
        ...
        self.n_old_classes = len(self.task_info['old_class'])  # 15-1: (DKD) : 16 ([1,2,3, .., 16]) (MiB, PLOP) : 17 ([0,1,2,3, .., 16])
        self.n_new_classes = len(self.task_info['new_class'])  # 15-1: (DKD) : 1  ([17])            (MiB, PLOP) : 1  ([17])
        ...
        if config.resume is not None:
            self._resume_checkpoint(config.resume, config['test'], config['linear_probe'], config['pr_curve'])

        pos_weight_cur = torch.ones([len(self.task_info['new_class'])], device=self.device) * self.config['hyperparameter']['pos_weight']
        pos_weight_prev = torch.ones([len(self.task_info['old_class'])], device=self.device) * self.config['hyperparameter']['pos_weight']
        
        self.BCELoss = WBCELoss(pos_weight=pos_weight_cur, n_old_classes=self.n_old_classes + 1, n_new_classes=self.n_new_classes)
        self.memBCELoss = BCELoss_DKD(pos_weight=pos_weight_prev)
        self.ACLoss = ACLoss()
...
class Trainer_incremental(Trainer_base):
    ...
    def __init__(
        ...
        self.train_metrics = MetricTracker(
            'loss', 'loss_mbce', 'loss_membce', 'loss_kd', 'loss_dkd_pos', 'loss_dkd_neg', 'loss_ac', 
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            if self.en_mem:
                ...
                data['image'][rand_index, ...] = mem_data['image'][rand_index, ...]
                data['label'][rand_index, ...] = mem_data['label'][rand_index, ...]
            ...
            with torch.cuda.amp.autocast(enabled=self.config['use_amp']):
                ...
                # [|Ct|]
                if self.en_mem:
                    logit_cur = logit[cur_data_idx, ...]
                    # feat_cur = features['attentions'][-1][cur_data_idx, ...]
                    label_cur = data['label'][cur_data_idx, ...]
                    
                    logit_mem = logit[mem_data_idx, ...]
                    # feat_mem = features['attentions'][-1][mem_data_idx, ...]
                    label_mem = data['label'][mem_data_idx, ...]
                    
                    # feat_old_cur = features_old['attentions'][-1][cur_data_idx, ...]
                    # feat_old_mem = features_old['attentions'][-1][mem_data_idx, ...]
                    
                    loss_mbce = self.BCELoss(
                        logit_cur[:, -self.n_new_classes:],  # [N, |Ct|, H, W]
                        label_cur,                # [N, H, W]
                    ).mean(dim=[0, 2, 3])
                    
                    loss_membce = self.memBCELoss(
                        logit_mem[:, 1:self.n_old_classes + 1], # [N, |C0:t-1|, H, W]
                        label_mem,               # [N, H, W]
                    ).mean(dim=[0,2,3])
                    ...
                    loss_membce = torch.tensor(0.)
                    
                
                # [|C0:t-1|]
                loss_kd = self.KDLoss(
                    logit[:, 1:self.n_old_classes + 1],  # [N, |C0:t-1|, H, W]
                    logit_old[:, 1:].sigmoid()       # [N, |C0:t-1|, H, W]
                ).mean(dim=[0, 2, 3])
                ...
                loss = self.config['hyperparameter']['mbce'] * loss_mbce.sum() + self.config['hyperparameter']['kd'] * loss_kd.sum() + \
                    ...
                    self.config['hyperparameter']['ac'] * loss_ac.sum() + self.config['hyperparameter']['mbce'] * loss_membce.sum() 
            ...
            self.train_metrics.update('loss', loss.item())
            self.train_metrics.update('loss_mbce', loss_mbce.mean().item())
            self.train_metrics.update('loss_membce', loss_membce.mean().item())
